...
export async function getAllProductsForHome(preview) {
  const data = await fetchAPI(
    `
    {
      allProducts(orderBy: _updatedAt_DESC, first: 40, filter: {cover: {neq: null}}) {
        productName
        subname
        slug
        rating
        ...
        platform
        cover {
          responsiveImage(imgixParams: {fm: jpg, fit: clip, w: 220, h: 220 }) {
            ...responsiveImageFragment
          }
        }
        heroimg {
          responsiveImage(imgixParams: {fm: webp, fit: clamp, w: 0.5, h: 0.5 }) {
            ...responsiveImageFragment
          }
        }
      }
...
export async function getProductAndMoreProducts(slug, preview) {
  ...
  query ProductBySlug($slug: String) {
    ...
    moreProducts: allProducts(orderBy: updatedAt_DESC, first: 10, filter: {slug: {neq: $slug}, cover: {neq: null}}) {
      productName
      subname
      slug
      rating
