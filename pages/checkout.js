              ...
              {state.map((cartItem) => (
                <div className="w-full bg-gradient-to-b rounded-md from-blue-100 to-transparent p-4 flex hover:shadow-lg shadow-md mb-4 flex-wrap items-center justify-between">
                  ...
                  <div className="w-1/3 md:w-1/5 flex items-center text-right text-sm">
                    <button
                      onClick={() =>
                        handleAdjustQuantity(cartItem.id, "DECREASE")
                      }
                    ...
                    <button
                      onClick={() =>
                        handleAdjustQuantity(cartItem.id, "INCREASE")
                      }
                      className={`focus:outline-none `}
                    >
                  ...
                  {cartItem.quantity > 1 && (
                    <div className="w-full my-2 text-sm text-gray-700 text-right">
                      Buying {cartItem.quantity} items at {cartItem.price} :-
                      each
                    </div>
                  )}
                </div>
                  ...
                  <button className="bg-blue-600 shadow-xl translate-y-0 hover:-translate-y-px hover:shadow-2xl duration-150 rounded-md px-6 transform transition py-2 text-lg text-white"><Link href="/thanks">Order</Link></button>
                  </div>
            </>
            :
            <p className="text-center text-lg">Your shopping cart is empty. Add some items before checking out. Click <Link href="/"><a className="border-b border-blue-600 border-dashed">here</a></Link> to return home.</p>
            }
            </div>
          </div>
        </Container>
